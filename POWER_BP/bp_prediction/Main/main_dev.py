...
if not os.path.exists('graficos_dev/graficos_N4_P3'):
    os.makedirs('graficos_dev/graficos_N4_P3')
...
for dim in n:
    ...
    for test_index in patient_idx:
        ...
        hiper_filtrada = filtfilt(b, a, test_patient)
...
############################################################################################################
        ...
        for horizonte in pred_horizon:
            for idx in range(0, len(derivada)-dim-horizonte+1):

                test_patient_input = derivada[idx:idx+dim] # input para predicao
                test_patient_output = derivada[idx+dim+horizonte-1] # output real
                
                predictions, error_code = predict_bp(test_patient_input, horizonte)

                if horizonte == 1:

                    indices_positivos = np.where(predictions[0] > 0)[0]
                    momentos_crescentes = [i for i in indices_positivos if filtered_hiper[i] > 140]

                    pre.append(momentos_crescentes)
            ...
            plt.plot(range(1, len(derivada)+1), derivada, label="Actual")
            plt.plot(range(dim+horizonte ,len(derivada)+1), pre, label="Predict")
            plt.axhline(y=linha_alta, color='r', linestyle='--')
            plt.title('Paciente {}'.format(test_index+1))
            plt.xlabel('Dia')
            plt.ylabel('Nível de hipertensão')
            plt.savefig('graficos_dev/graficos_N4_P3/paciente{}.png'.format(test_index+1))
            plt.close()
